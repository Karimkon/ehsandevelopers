<x-layouts.admin>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-xl font-display font-bold text-white">Edit Category</h2>
            <p class="text-gray-400 text-sm mt-1">Update "{{ $serviceCategory->name }}"</p>
        </div>
    </div>

    <!-- Form -->
    <div class="admin-card max-w-2xl">
        <form action="{{ route('admin.categories.update', $serviceCategory) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" name="color" id="color" value="{{ old('color', $serviceCategory->color ?? '#10b981') }}"
                           class="h-10 w-14 rounded border border-surface-600 bg-surface-800 cursor-pointer">
                    <input type="text" id="color_text" value="{{ old('color', $serviceCategory->color ?? '#10b981') }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="admin-input w-full" placeholder="Brief description of this category...">{{ old('description', $serviceCategory->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
